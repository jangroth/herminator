...
**Status:** Done — deployed, live, and verified against the running cluster on 2026-08-22. All acceptance criteria checked.
**Date:** 2026-08-14
**Revised:** 2026-08-16 — review pass against the live cluster, plus a same-day second pass fixing internal contradictions found in spec review; see "Revisions" below. **2026-08-17** — chart built and validated (`chart/`); Constraint C resolved; two spec assumptions corrected (no Dex client secret; wrong Tailscale state env var name). See "Implementation Status" below. **2026-08-18** — real `API_SERVER_KEY` and a reusable/tagged/non-ephemeral Tailscale auth key generated and sealed into the chart (rollout step 5 done). **2026-08-22** — rollout steps 6–9 done (chart 0.1.1→0.1.8 fixing crashloop, Host header, tailscale authkey, dashboard/OAuth bind, asymmetric routing, and config.yaml seeding along the way — see CHANGELOG.md); acceptance checklist walked and verified live; spec closed.
...
- **The loopback-bound dashboard breaks default probes.** kubelet directs `tcpSocket`/`httpGet` probes at the *pod IP*, so a probe on `:9119` would always fail once the dashboard binds `127.0.0.1` (Constraint B) — instant crashloop. Probes moved to `:8642` (hermes container) and `:443` (nginx). *(Constraint B's loopback bind was itself reversed 2026-08-19 — see that section — but the probe placement is unaffected either way and stayed on `:8642`/`:443`.)*
...
- ~~Bind the dashboard to `127.0.0.1`.~~ **Reversed 2026-08-19 (chart 0.1.5, `herminator@cd67b75`).** This was live briefly and turned out backwards: `HERMES_DASHBOARD_HOST=127.0.0.1` doesn't just narrow the listen scope — hermes itself reads it as "no external access needed" and disables its own Dex OAuth gate (`__HERMES_AUTH_REQUIRED__=false`), plus applies loopback-only WebSocket Origin rules that fought nginx's proxying (reconnect-loop, code 1006). That shipped a dashboard with **no auth at all**, the opposite of what this fix was for. Dashboard now binds `0.0.0.0` so the OAuth gate actually engages. The inbound-exposure risk this bullet was guarding against is still covered — by `--shields-up` above and by the dashboard having no Service pointing at `:9119` except through the nginx/Dex front door — neither of which depended on the loopback bind. See [DECISIONS.md #007's 2026-08-22 amendment](../../DECISIONS.md#007).
...
6. ~~Push this repo's chart first...~~ **Done.**
7. ~~Land the homekube-apps changes (Dex static client + new `hermes.yaml` Application), sync, verify Dex serves the new client.~~ **Done** — homekube DECISIONS.md #052, `homekube-apps@9b5a358`.
8. ~~Sync the hermes Application.~~ **Done.**
9. ~~Walk the acceptance list below.~~ **Done 2026-08-22** — see checked items and evidence below.
10. Update homekube's docs per Decision 6. **Done** — homekube README "Deployed Components"/"Resource Budget" tables, CHANGELOG.md, DECISIONS.md #052 all carry the Hermes entries.

## Acceptance

Verified live against the running cluster on 2026-08-22 (pod `hermes-59c88d9869-z5b8p`, 24h uptime, chart 0.1.8).

- [x] `kubectl get pods -n hermes` — Running, all three containers ready. Confirmed: `3/3 Running`, 0 restarts.
- [x] `https://192.168.86.246` from a Tailscale-connected client redirects to Dex login; completing it lands back on the authenticated hermes dashboard. Confirmed via nginx access logs: ongoing live browser sessions from a tailnet client (`/api/auth/ws-ticket` → 200, `/api/mcp/*`, `/channels` all 200) — this is real day-to-day usage, not just a synthetic check.
- [x] hermes completes OIDC discovery against `https://pi0.taild13083.ts.net/dex` with no x509 error in its logs (Constraint C). No Dex/OIDC-related x509 errors in container logs — the only `CERTIFICATE_VERIFY_FAILED` entries are an unrelated Telegram-plugin egress issue (see note below), not Dex/CA-trust related.
- [x] No basic auth anywhere — old `HERMES_DASHBOARD_BASIC_AUTH_*` vars gone entirely. Confirmed: no `BASIC_AUTH` string anywhere in the live Deployment spec.
- [x] From another tailnet device, `hermes-aperture:9119` and `:8642` are **not** reachable — `--shields-up` holds and the dashboard has no non-TLS, non-OIDC path (Constraint B). Directly tested from another tailnet device (`darth`, `100.93.165.84`): both `100.84.127.40:9119` and `:8642` connection-timed-out. (tailscaled's health check does flag an iptables `filter`-table init failure on the Pi's kernel — worth a closer look eventually — but the live connectivity test confirms shields-up is in fact blocking inbound.)
- [x] `/opt/data` actually persists across a pod restart (PVC bound, Longhorn-backed). Confirmed: `hermes-data` PVC `Bound`, 5Gi, 2d17h age — outlived multiple pod recreates across chart 0.1.1→0.1.8.
- [x] A rollout completes rather than deadlocking on the RWO volume — proves `strategy: Recreate`. Confirmed: live Deployment has `strategy.type: Recreate`; chart has rolled 0.1.1 through 0.1.8 (8 releases, each a fresh rollout) without a deadlock.
- [x] Tailscale sidecar shows up as its own device in the tailnet admin console; hermes can reach Aperture and complete an actual LLM request through it. Confirmed: `hermes-aperture` (`100.84.127.40`) listed in `tailscale status`; the `ai` (Aperture) peer shows `active` with real tx/rx traffic; gateway logs show live session activity against a real model (`nvidia/nemotron-3-ultra-550b-a55b:free`).
- [x] Sidecar survives a pod restart without creating a duplicate/orphaned tailnet device. Confirmed: exactly one `hermes-aperture` device in `tailscale status` despite 8 chart-version rollouts (each a pod recreate) — no orphans accumulated, state persistence via `TS_KUBE_SECRET` works and ArgoCD's `ignoreDifferences` on that Secret is holding.
- [x] ArgoCD shows the app Synced/Healthy. Confirmed: `Synced` / `Healthy`.
- [x] No plaintext high-value credentials in git. Confirmed: `chart/templates/sealedsecret.yaml` holds only `encryptedData` ciphertext for `API_SERVER_KEY`/`TS_AUTHKEY`; no `secret.yaml` template exists (no Dex client secret — public PKCE client); grepped the chart tree for plaintext key/PEM patterns, none found.

**Note (out of MVP scope, worth a follow-up issue):** container logs show the Telegram platform plugin failing with `CERTIFICATE_VERIFY_FAILED: self-signed certificate in certificate chain` on every connect attempt — likely `NODE_EXTRA_CA_CERTS`/`SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE` (added for Constraint C's Dex trust) now also applying to Telegram's egress path, which doesn't go through `homekube-ca`. Unrelated to this spec's acceptance criteria (Telegram isn't in MVP scope) but a real live issue — recommend filing it.
